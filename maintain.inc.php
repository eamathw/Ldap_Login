...
class Ldap_Login_maintain extends PluginMaintain
{
    ...
    public function install($plugin_version, &$errors = [])
    {
        ...
        if (! ld_table_exist()) { // new install or from old situation
            ...
            // prepare sql-table
            $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Created SQL-table');
            ld_sql('create', 'create_table');
            $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Created SQL-data from default values');
            ...
            if (file_exists(LDAP_LOGIN_PATH . '/data.dat') && ! file_exists(LDAP_LOGIN_PATH . '/config/data.dat')) { // only in root not in .config/
                ...
                $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Moved data.dat');
            }

            // future, in new situation (inactivated plugin)
            if (file_exists(LDAP_LOGIN_PATH . '/config/data.dat')) {
                $ld_log->debug('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> loading ./config/data.dat ');
                $ld_log->debug('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> function load_old_config ');
                $ld_config->loadOldConfig(); // will overwrite default values
                unlink(LDAP_LOGIN_PATH . '/config/data.dat'); // delete data.dat
                $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> deleted ./config/data.dat ');
            }
        }
        else {
            $ld_config->loadDefaultConfig();
            $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Default config loaded ');
            $ld_config->loadConfig($merge = true);
            $ld_log->debug('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Merged old config');
            ld_sql('create', 'create_data', $ld_config->getAllValues());
            $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Expanded database');
            ld_sql('update', 'update_sql_structure');
            $ld_log->debug('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Added Column with timestamps');
        }
        $ld_log->debug('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Saving config');
        $ld_config->saveConfig();
        $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> plugin installed');
        $this->installed = true;
    ...
    public function activate($plugin_version, &$errors = [])
    {
        ...
        $ld_config->loadDefaultConfig();
        $ld_config->loadConfig($merge = true);
        $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> activate');
        ...
        if (! $this->installed) {
            ...
            $ld_log->debug('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> [Maintain.inc.php/Install] ');
            $this->install($plugin_version, $errors); // then install
            $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> plugin activated');
        }
        unset($ld_config, $ld_log);
    }
    ...
    public function deactivate()
    {
        ...
        $ld_log->setHandlers($handlerArray);
        $ld_config = new Config();
        $ld_log->warning('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> Check value of "allow_user_registration" as no user is currently able to register.');
        $ld_log->info('[' . basename(__FILE__) . '/' . __FUNCTION__ . ':' . __LINE__ . ']> deactivated');
        unset($ld_config, $ld_log);
    }
