...
$template->set_filenames(['plugin_admin_content' => dirname(__FILE__) . '/test.tpl']);
$template->assign(
    [
...
if (isset($_POST['check_ldap'])) {
    ...
    if ($ld_config->getValue('ld_anonbind') == 0) {
        ...
        $row = ['id' => (get_userid($_POST['USERNAME']) ? get_userid($_POST['USERNAME']) : get_userid_by_email($_POST['USERNAME']))];
